...
@dataclass
class GameState:
    """Persistent game state structure tracking global metrics and active sessions"""
    ...
    # --- ADD THESE ACTIVE SESSION PROPERTIES ---
    active_chapter_number: Optional[int] = None
    active_time_remaining: Optional[int] = None
    active_score: Optional[int] = None
    active_attempts: int = 0
    active_trapped: bool = False
    active_tools_used: List[str] = field(default_factory=list)
    active_filters: List[str] = field(default_factory=list)
    active_logs: List[Dict[str, Any]] = field(default_factory=list)
    active_malicious_ips: List[str] = field(default_factory=list)
    ...
    if SAVE_FILE.exists():
        try:
            with open(SAVE_FILE, 'r') as f:
                ...
                # Manually build the GameState to support old/missing keys cleanly
                return GameState(
                    tutorial_completed=data.get("tutorial_completed", False),
                    highest_chapter_unlocked=data.get("highest_chapter_unlocked", 1),
                    total_score=data.get("total_score", 0),
                    tools_unlocked=data.get("tools_unlocked", {"virustotal": False, "siem": False, "regex": False}),
                    achievements=data.get("achievements", []),
                    high_scores=data.get("high_scores", []),
                    
                    # Session Restoration Fallbacks
                    active_chapter_number=data.get("active_chapter_number"),
                    active_time_remaining=data.get("active_time_remaining"),
                    active_score=data.get("active_score"),
                    active_attempts=data.get("active_attempts", 0),
                    active_trapped=data.get("active_trapped", False),
                    active_tools_used=data.get("active_tools_used", []),
                    active_filters=data.get("active_filters", []),
                    active_logs=data.get("active_logs", []),
                    active_malicious_ips=data.get("active_malicious_ips", [])
                )
        except Exception:
            pass
...
class GameScreen(Screen):
    ...
    def __init__(self, game_state: GameState, chapter: ChapterConfig):
        ...
        # Check if we should restore an active saved snapshot for this chapter
        if self.game_state.active_chapter_number == chapter.number and self.game_state.active_logs:
            self.logs = self.game_state.active_logs
            self.malicious_ips = self.game_state.active_malicious_ips
            self.time_remaining = self.game_state.active_time_remaining
            self.score = self.game_state.active_score
            self.attempts = self.game_state.active_attempts
            self.trapped = self.game_state.active_trapped
            self.tools_used = set(self.game_state.active_tools_used)
            self.active_filters = self.game_state.active_filters
        else:
            # Generate clean level parameters
    ...
    async def handle_menu_action(self, action: Optional[str]) -> None:
        ...
        if action == "save":
            ...
            # Textual expects the Selector String first, then the Type
            log_panel = self.query_one("#log-panel", ScrollableContainer)
            log_panel.remove_children()
            log_panel.mount(Static(self.format_logs()))
            
            output_log.write("🔄 [bold #f1fa8c]System Traffic Data Stream Repopulated/Reloaded.[/]")
...
class ForensicsBriefingApp(App):
    ...
    SUB_TITLE = "Version 1.1.0 | CLASSIFIED"
    ...
    async def on_mount(self) -> None:
        """Initialize the application and handle session routing"""
        if not self.game_state.tutorial_completed:
            self.push_screen(WelcomeModal(), callback=self.handle_welcome_response)
        elif self.game_state.active_chapter_number is not None:
            # Reconstruct the active level layout directly from file state
            chapter = CHAPTERS[self.game_state.active_chapter_number - 1]
            await self.push_screen(GameScreen(self.game_state, chapter))
        else:
